...
describe("profile.updateMedia owner controls", () => {
  ...
  it("routes a hidden visibility change to the authenticated media owner only", async () => {
    await appRouter
      ...
      .profile.updateMedia({ mediaId: 12, visibility: "hidden" });
    expect(mocks.updateProfileMedia).toHaveBeenCalledWith(7, 12, {
      visibility: "hidden",
      featured: undefined,
    });
  });
  it("routes featured selection to the authenticated media owner, allowing the database layer to clear the previous feature", async () => {
    await appRouter.createCaller(context).profile.updateMedia({
      mediaId: 19,
      featured: true,
      visibility: "public",
    });
    expect(mocks.updateProfileMedia).toHaveBeenCalledWith(7, 19, {
      visibility: "public",
      featured: true,
    });
  });
