...
const buildEndpoint = (rpc: string): string => {
  if (!ENV.serviceApiUrl) {
    throw new TRPCError({
      ...
      message: "Heartbeat service URL is not configured (SERVICE_API_URL).",
    });
  }
  if (!ENV.serviceApiKey) {
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message: "Heartbeat service API key is not configured (SERVICE_API_KEY).",
    });
  }
...
const callService = async <T>(
  ...
  if (!response.ok) {
    ...
    throw mapServiceError(response, detail, rpc);
  }
...
export async function createHeartbeatJob(
  ...
  return callService<{ taskUid: string; nextExecutionAt?: string | null }>(
    "CreateHeartbeatJob",
    {
      name: job.name,
...
export async function updateHeartbeatJob(
  ...
  return callService<{ nextExecutionAt?: string | null }>(
    "UpdateHeartbeatJob",
    body,
    userSession
  );
...
export async function deleteHeartbeatJob(
  ...
  await callService("DeleteHeartbeatJob", { taskUid }, userSession);
}
