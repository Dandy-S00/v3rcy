...
export default function CommunityFeed() {
  ...
  const createPost = trpc.community.create.useMutation({ onSuccess: () => { setBody(""); utils.community.feed.invalidate(); toast.success("Your post is live."); }, onError: (error) => toast.error(error.message) });
  ...
  return (
    <main className="community-page page-shell">
      <section className="community-hero glass-panel">
        <div><p className="eyebrow">Private community</p><h1>A softer place to connect.</h1><p className="muted-copy">Share thoughts, discover common ground, and stay in control of your privacy.</p></div>
        <ShieldCheck aria-hidden="true" size={34} />
      </section>
      <section className="community-layout">
        <aside className="community-sidebar glass-panel"><Sparkles size={22} /><h2>Community guidelines</h2><p>Keep posts respectful, adult-only, and free from personal contact details. Report anything that feels unsafe.</p><Link href="/safety" className="text-link">Review safety tools</Link></aside>
        <div className="feed-column">
          <section className="composer glass-panel"><textarea value={body} onChange={(event) => setBody(event.target.value)} maxLength={3000} placeholder="Share something with the community…" aria-label="Post text" /><div className="composer-footer"><label className="toggle-label"><input type="checkbox" checked={commentsEnabled} onChange={(event) => setCommentsEnabled(event.target.checked)} /> Allow comments</label><button className="primary-button" onClick={submitPost} disabled={createPost.isPending || !body.trim()}><Send size={16} /> Post</button></div></section>
          ...
          {feed.data?.posts.map((post) => <article className="post-card glass-panel" key={post.id}>
            <header className="post-header"><div className="avatar-placeholder" aria-hidden="true">{post.author.name.slice(0, 1).toUpperCase()}</div><div><strong>{post.author.name}</strong><p>{post.author.city ?? "Private location"} · {formatDate(post.createdAt)}</p></div></header>
            ...
            {post.commentsEnabled ? <form className="comment-form" onSubmit={(event) => submitComment(event, post.id)}><input name="comment" placeholder="Write a respectful comment…" maxLength={1000} aria-label="Comment" /><button className="icon-button" aria-label="Send comment"><Send size={16} /></button></form> : <p className="comments-disabled">Comments are disabled by the author.</p>}
          ...
          <div className="feed-pagination"><button className="secondary-button" disabled={page === 1} onClick={() => setPage((value) => value - 1)}>Previous</button><span>Page {page}</span><button className="secondary-button" disabled={!feed.data?.hasMore} onClick={() => setPage((value) => value + 1)}>Load more</button></div>
        </div>
      </section>
    </main>
  );
