...
export const userProfiles = mysqlTable(
  "userProfiles",
  {
    ...
    verificationStatus: mysqlEnum("verificationStatus", ["none", "email", "id"])
      .default("none")
      .notNull(),
    accountStatus: mysqlEnum("accountStatus", ["active", "review", "suspended"])
      .default("active")
      .notNull(),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
...
export const listings = mysqlTable(
  "listings",
  {
    ...
    city: varchar("city", { length: 80 }).notNull(),
    verificationRequired: mysqlEnum("verificationRequired", [
      "none",
      "email",
      "id",
    ])
      .default("none")
      .notNull(),
    visibility: mysqlEnum("visibility", ["live", "removed", "archived"])
      .default("live")
      .notNull(),
  ...
  table => [
    index("listings_visibility_city_idx").on(table.visibility, table.city),
    index("listings_owner_idx").on(table.ownerUserId),
  ]
);
...
export const profileMedia = mysqlTable(
  "profileMedia",
  {
    ...
    visibility: mysqlEnum("visibility", ["public", "hidden"])
      .default("public")
      .notNull(),
  ...
  table => [
    index("profile_media_user_idx").on(
      table.userId,
      table.sortOrder,
      table.createdAt
    ),
...
export const safetySignals = mysqlTable(
  "safetySignals",
  {
    id: int("id").autoincrement().primaryKey(),
    ...
    note: text("note"),
    status: mysqlEnum("status", ["received", "under_review", "resolved"])
      .default("received")
      .notNull(),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
  },
...
export const moderationActions = mysqlTable(
  ...
  table => [
    index("moderation_actions_admin_idx").on(
      table.adminUserId,
      table.createdAt
    ),
  ]
...
export const actionRateLimits = mysqlTable(
  "actionRateLimits",
  {
    id: int("id").autoincrement().primaryKey(),
    userId: int("userId")
  ...
  table => [
    uniqueIndex("action_rate_limits_user_action_unique").on(
      table.userId,
      table.actionType
    ),
    index("action_rate_limits_window_idx").on(table.windowStartedAt),
  ]
);
